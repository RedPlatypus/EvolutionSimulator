...
class TableCell{
  ...
  TableCell(float x, float y, float width, float height, Ant cellD){
    ...
    this.horizSpacing = width / (this.numOfCells + 0.5); //num of width / (cols + 0.5)
  }
  ...
  void drawCell(){
    ...
    //if num of columns == 4 don't show this
    if(this.numOfCells != 4){
      text("energy", 0,0);
      translate(this.horizSpacing, 0);//font size + 2
    }
    text("age", 0,0);
    translate(this.horizSpacing, 0);//font size + 2
    text("food :)", 0,0);
