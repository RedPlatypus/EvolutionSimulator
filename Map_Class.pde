class Map{
  ...
  float area;
  float probableCarryingCapacity; // number of ants the land can probably support ( used in brainInputs )
  ...
  Map(float width, float height, int numAnts){
    ...
    this.probableCarryingCapacity = map(this.area, 0, 1000000, 15, 800);
    //println(this.area);
    //println(this.probableCarryingCapacity);
  ...
  ArrayList<Ant> getSpawingAntsInProximity(Ant mother){
    ArrayList<Ant> proxyAnts = new ArrayList<Ant>();
    
    for(Ant ant: ants){
      if(ant == mother){
        continue; // we don't want to include the mother initiating the birth
      } else {
        if(WORLD.physicsWorld.isWithinDistance(mother.body, ant.body, BREEDING_DISTANCE)){
          //check if of breeding size
          if(ant.energy > BREEDING_ENERGY){
            proxyAnts.add(ant);
          }
        }
      }
  ...
  void createMap(){
    ...
    for(int x = 0; x < cols; x++){
      ...
      for(int y = 0; y < rows; y++){
        ...
        tiles[x][y] = new Tile(MAP_SCALE, noise(xOffH, yOffH) * 255, (noise(xOffS, yOffS) + 0.5) * INITIAL_FOOD, brightVal, x, y);
        yOffH += HUE_MAP_NOISE;
