...
final float MAX_GROWTH_RATE = 0.15;
final float MAX_DEATH_RATE = 0.03;
...
//ANT SPECIFIC
final float ENERGY_REQUIRED_MOVE = 0.8;//1
final float ENERGY_REQUIRED_SWIM = 1.8;//3
...
final float MAX_BITE_SIZE = 6.0; //bite size can't be changed by them.
final float SCOPE = 70.0; // How picky is the ant? 63.5 (255 / 4) for eating exactly 1/2 the spectrum. gets hurt by other 1/2 (Higher number for a less picky ant, that also gets hurt less!)
...
final float MAX_VELOCITY = 0.5;//0.15
...
final int OUTN = 14;
final float LEARNING_RATE = 0.13;
