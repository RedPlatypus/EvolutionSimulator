class Ant {  
  ...
  color myColor = color(random(0,255),150,150), tempColor = color(255,255,255);
  int generation;
...
//constructor - initalizes the variables or the setting of the object the class creates
  ...
  Ant(PVector location, int generation){ 
    ...
    if(SMART_ANTS){
      _input = new BrainInput(LEARNING_RATE, this);
    } else {
  ...
  Ant(PVector pos, BrainInput brain, int gen, float mouthHue, color antColor){
    this.energy = STARTING_ENERGY;
    convertEnergyToBodySize(this.energy);
    body = new PhysicsCircle(bodySize / 2, pos);
    this._input = brain;
  ...
  Ant(Ant parent){
    ...
    this.mHue = parent.mHue + (randomGaussian() * 2);
    if(this.mHue > 255){
  ...
  boolean isBreedingSize(){
    //return true if energy > breeding size
    if(this.energy > this.breedingSize){
      return true;
    }
    return false;
  }
  
  void inBreedingDistance(Boolean birthAntPresent){
    //we don't get side effects from this... but we do from the below solution!
   if(birthAntPresent){
      birthAntNearby = true;
      //myColor = color(127,100,200);
    }else{
      birthAntNearby = false;
      //myColor = color(250,100,200);
    }
  }
  ...
  void giveBirth(){
    ...
    ArrayList<Ant> antsAlsoBreeding = WORLD.worldMap.getSpawingAntsInProximity(this); // returns a list of ants in the area that have energy enough to breed!
    //if(antsAlsoBreeding.size() <= 0){
    //  println("FOR TESTING: Ant class birth: only allows breeding if another 1 or more ants in location");
    //  return;
    //}
    BirthPod bp = new BirthPod();
    bp.add(this);
    if(antsAlsoBreeding.size() > 0){
      for(Ant ant: antsAlsoBreeding){
        bp.add(ant);
      }
    }
    bp.birth();
