final float SCREEN_WIDTH = 1920;//check size in setup is same! (1920)
final float SCREEN_HEIGHT = 1000;//check size in setup is same! (1000)
final float GUI_PERCENT_OF_SCREEN = 0.15;

final World WORLD = new World(SCREEN_WIDTH * (1-GUI_PERCENT_OF_SCREEN), SCREEN_HEIGHT);
final GUI antGUI = new GUI(SCREEN_WIDTH * (1-GUI_PERCENT_OF_SCREEN), 
                            0, 
                            SCREEN_WIDTH * GUI_PERCENT_OF_SCREEN, 
                            SCREEN_HEIGHT);
final InputComponent playerInput = new InputComponent();
//final NeuralNet brain = new NeuralNet(); //need to give to every ant!

void setup() {
  size(1920,1000); // needs to be same as screen_width & screen_height!
